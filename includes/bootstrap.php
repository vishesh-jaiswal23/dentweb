...
function validate_and_save_ai_settings(array $settings, ?PDO $db = null): void
{
    $db = $db ?? get_db();

    $apiKey = $settings['api_key'] ?? '';
    $textModel = $settings['gemini_text_model'] ?? 'gemini-1.5-flash';
    ...
    if ($httpCode !== 200) {
        throw new RuntimeException('Gemini API validation failed. Please check your API key and model code.');
    }

    foreach ($settings as $key => $value) {
        set_setting('ai_setting_' . $key, $value, $db);
    }
}

function stream_gemini_response(string $prompt, ?PDO $db = null): void
{
    $db = $db ?? get_db();

    $apiKey = get_setting('ai_setting_api_key', $db);
    $textModel = get_setting('ai_setting_gemini_text_model', $db) ?? 'gemini-2.5-flash';
